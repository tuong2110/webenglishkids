...
function checkLogin()
{
    ...
    if (!isset($_SESSION["account"])) {
        ...
        // Kiểm tra linh vật (chỉ kiểm tra nếu không phải trang chọn linh vật và không phải AJAX request)
        $currentPage = $_SERVER['REQUEST_URI'] ?? '';
        $isAjax = !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest';
        $isAjax = $isAjax || (!empty($_POST) && isset($_POST['type'])); // Kiểm tra nếu có POST với type
        
        if (strpos($currentPage, 'ChonLinhVat') === false && strpos($currentPage, 'KhoiTaoTaiKhoan') === false && !$isAjax) {
            $taiKhoan = check_string($_SESSION["account"]);
            ...
            if (!$checkLinhVat) {
                return die('<script type="text/javascript">
                    setTimeout(function(){ location.href = "' . BASE_URL('Page/ChonLinhVat') . '" }, 0);
                    </script>
                    ');
            }
        }
