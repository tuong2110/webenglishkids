...
if ($_POST['type'] == 'GetNewWord') {
    ...
    if (count($danhSachTuVungChuaHoc) == 0 ||  $checkToken["SoCauHienTai"] == 5 || $checkToken["TienTrinh"] == 100) {
        // Tặng điểm thưởng khi hoàn thành bài học (ít)
        $diemThuong = 5; // Điểm thưởng nhỏ
        ...
        $HoatDong->insertHoatDong([
            'MaLoaiHoatDong' => 2,
            'TenHoatDong' => 'Hoàn thành bài học',
            'NoiDung' => 'Hoàn thành bài học và nhận được ' . $diemThuong . ' điểm thưởng',
            'TaiKhoan' => $taiKhoan
        ]);
        ...
        $result = array(
            ...
            'message' => 'Hoàn thành bài học! Nhận được ' . $diemThuong . ' điểm thưởng!'
        );
    } else {
...
if ($_POST['type'] == 'PracticeDifficultWord') {
    try {
        ...
        if (!$checkToken || count($checkToken) == 0) {
            throw new Exception(getMessageError2('Token không tồn tại. Khóa học: ' . $khoaHoc . ' - Vui lòng tải lại trang!'));
        }
        // Hoàn thành
        if ($checkToken["TienTrinh"] == 100) {
            $result = array(
...
if ($_POST['type'] == 'PracticeWordType1') {
    try {
        ...
        if ($typeOnTap == "tuKho") {
            ...
            if (!$checkPracticeToken || count($checkPracticeToken) == 0) {
                throw new Exception(getMessageError2('Token không tồn tại. Vui lòng tải lại trang!'));
            }
        ...
        // Kiểm tra xem có học từ vựng này hay chưa
        $checkHocTuVung = $Database->get_row("select * from hoctuvung where TaiKhoan = '" . $_SESSION['account'] . "' and MaTuVung = '" . $checkToken["MaTuVung"] . "'  and MaBaiHoc = '" . $checkToken["MaBaiHoc"] . "' and MaKhoaHoc = '" . $checkToken["MaKhoaHoc"] . "'");
        if (!$checkHocTuVung || count($checkHocTuVung) == 0) {
            throw new Exception(getMessageError2('Dữ liệu không tồn tại'));
        }
        ...
        if ($checkToken["MaTuVung"] != $userAnswer) {
            ...
            // Cập nhật đúng bảng tùy theo typeOnTap
            if ($typeOnTap == "tuKho") {
                $Database->update("ontaptuvungkho", ['TienTrinh' => $tienTrinh, 'SoCauHienTai' => $soCauHienTai], "Token = '" . $practiceToken . "' AND `TaiKhoan` = '" . $_SESSION['account'] . "' ");
            } else {
...
if ($_POST['type'] == 'HienThiDapAn') {
    try {
        ...
        // Kiểm tra xem token có tồn tại trong một trong hai bảng không
        if ((!$checkPracticeToken1 || count($checkPracticeToken1) == 0) && (!$checkPracticeToken2 || count($checkPracticeToken2) == 0)) {
            throw new Exception(getMessageError2('Token không tồn tại. Vui lòng tải lại trang!'));
        }
        $checkToken = $Database->get_row("SELECT * FROM ontaploai1 WHERE Token = '" . $token . "' AND `TaiKhoan` = '" . $_SESSION['account'] . "'");
        if (!$checkToken || count($checkToken) == 0) {
...
if ($_POST['type'] == 'DanhDauTuKhoOnTap') {
    try {
        ...
        if (!$checkTokenOnTap || count($checkTokenOnTap) == 0) {
            throw new Exception(getMessageError2('Token không tồn tại. Vui lòng tải lại trang!'));
        }
